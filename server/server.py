...
import hmac             # for message integrity
import hashlib          # to use hash functions
        ...
        if blk_count == 0: #  1st block
            ...
            if cmd_GET_MENU in usr_cmd: # ask for menu
                ...
                while True:
                    ...
                    conn.send(read_bytes) #sends the message (aka menu)
                    ...
                    message_hmac = hmac.digest(key=session_key, msg=read_bytes, digest=hashlib.sha256)
                    conn.send(message_hmac) #sends the hmac of the message
